...
final classAttendanceProvider =
    FutureProvider.family<List<Attendance>, ({String classId, String date})>(
        (ref, args) async {
  ref.watch(realtimeSyncProvider);
  final repo = ref.read(attendanceRepositoryProvider);
...
final studentAttendanceProvider =
  ...
  ref.watch(realtimeSyncProvider);
  final repo = ref.read(attendanceRepositoryProvider);
  final result = await repo.getStudentAttendance(studentId: studentId);
  return result.fold((e) => throw Exception(e), (v) => v);
});
...
class MarkAttendanceNotifier extends FamilyNotifier<AsyncValue<void>, String> {
  @override
    ...
    result.fold(
      ...
      (_) {
        for (final record in records) {
          ...
          ref.invalidate(
            classAttendanceProvider((classId: arg, date: date)),
          );
          ref.invalidate(studentAttendanceProvider(studentId));
          ref.invalidate(studentAttendanceSummaryProvider(studentId));
        }
...
final markAttendanceProvider =
    NotifierProvider.family<MarkAttendanceNotifier, AsyncValue<void>, String>(
